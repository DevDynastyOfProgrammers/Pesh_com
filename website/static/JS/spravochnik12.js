...
// Функция для перенаправления на окно регистрации при клике на кнопку
document.getElementById('register-button').addEventListener('click', () => {
    window.location.href = 'register.html'; // Перенаправление на страницу map.html
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    eventCards.forEach(card => {
        card.addEventListener('click', function() {
            const eventId = card.getAttribute('data-id'); // Получаем идентификатор мероприятия из атрибута data-id
            window.location.href = `info.html?id=${eventId}`; // Перенаправляем на страницу с информацией о мероприятии, передавая идентификатор через URL
        });
    });
});